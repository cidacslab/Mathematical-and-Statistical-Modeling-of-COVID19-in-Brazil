...
class SIR_EDO:
    ...
    def predict(self,x):
        ...
        if (self.beta == None or self.gamma == None):
            ...
            t_inc = 1
            t_range = np.arange(t_start, t_end + t_inc, t_inc)
            result_fit = spi.odeint(self.SIR_diff_eqs, (self.S0, self.I0, self.R0), t_range, args=(self.beta, self.gamma))
            
            return result_fit[:, 1]*self
        
    def getCoef(self):
        return [self.beta,self.gamma],['beta','gamma']
