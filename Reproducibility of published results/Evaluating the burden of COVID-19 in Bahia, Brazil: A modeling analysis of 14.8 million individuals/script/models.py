...
class SIR(Models):
    ...
    def __fit(self,x, y ,c1= 0.15, c2= 0.25, w = 0.7):
        ...
        if self.BetaChange!=0:
            if self.isFitDayBetaChange == True:
                ...
                pos.append(self.gamma)
                pos = np.array(pos)
                par=ps.backend.generators.create_swarm(20,dimensions=(self.BetaChange+1)*2,bounds=self.bound)
                for i in range(5):
                    par.position[i]=pos
                
                optimizer = ps.single.GlobalBestPSO(n_particles=20, dimensions=(self.BetaChange+1)*2, options=options,bounds=self.bound,init_pos=par.position)
                ...
                optimizer = ps.single.GlobalBestPSO(n_particles=20, dimensions=self.BetaChange+2, options=options,bounds=self.bound)
                cost, pos = optimizer.optimize(self._objectiveFunction, 1000,n_processes=self.nCores)
                beta = []
            ...
            optimizer = ps.single.GlobalBestPSO(n_particles=20, dimensions=2, options=options,bounds=self.bound)
            cost, pos = optimizer.optimize(self._objectiveFunction, 1000,n_processes=self.nCores)
            beta = [pos[0]]
            self.beta = np.array(beta)
            self.gamma = pos[1]
    ...
    def fit(self,x, y ,fittingByCumulativeCases=True, bound = ([0,1/21],[2,1/5]),stand_error=True, BetaChange=0, dayBetaChange = None,particles=100,itera=1000,c1= 0.5, c2= 0.3, w = 0.9, k=5,norm=1):
        '''
        ...
        if self.BetaChange!=0:
            if self.dayBetaChange==None:
                ...
                #__fitNBeta(self,dim,n_particles,itera,options,objetive_function,**kwargs)
                    pos,cost = self._Models__fitNBeta(dim=(self.BetaChange+1)*2,n_particles=self.particles,itera=self.itera,options=options,objetive_function=  self._objectiveFunction,BetaChange= self.BetaChange,bound=self.bound)
                else:
                    optimizer = ps.single.LocalBestPSO(n_particles=particles, dimensions=(self.BetaChange+1)*2, options=options,bounds=self.bound)
                    cost, pos = optimizer.optimize(self._objectiveFunction, itera,n_processes=self.nCores)
    ...
    def plot(self,local=None,InitialDate=None,CompartmentPlots=None,SaveFile=None):
        ...
        if CompartmentPlots==None:
            ...
            if self.BetaChange>0:
                for a in self.dayBetaChange:
                    ax.axvline(a, 0, 600,c='r',linestyle='--',label='Beta change')
        ...
        #Criar o layout onde os plots serao gerados. E nessa parte que se define o grid
            ...
            for k in range(len(ax)):
                ...
                if self.BetaChange >0:
                    for a in self.dayBetaChange:
                        ax[k].axvline(a, 0, 600,c='r',linestyle='--',label='Beta Change')
...
class SEIR(Models):
    ...
    def __cal_EDO_2(self,x,tVar,betaVar,gamma,mu,sigma):
            t_range = x
            def beta(t,tVar,betaVar):
                for i in range(len(tVar)):
                    if t<tVar[i]:
    ...
    def __residuals(self,coef):
        if (self.BetaChange!=0) & (self.dayBetaChange==None):
            ...
            for i in range(self.BetaChange):
                ...
                betaVar[i] = coef[self.BetaChange+i]
            betaVar[self.BetaChange] = coef[self.BetaChange*2]
            S,E,I,R = self.__cal_EDO_2(self.x,tVar,betaVar,coef[self.BetaChange*2+1],self.mu,coef[self.BetaChange*2+2])    
        elif self.dayBetaChange!=None:
            tVar = self.dayBetaChange
            betaVar = np.ones(self.BetaChange+1)
            for i in range(self.BetaChange+1):
                betaVar[i] = coef[i]      
            S,E,I,R = self.__cal_EDO_2(self.x,tVar,betaVar,coef[self.BetaChange+1],self.mu,coef[self.BetaChange+2])
        else:
    ...
    def __validateBound(self, bound):
        ...
        if (self.BetaChange!=0) & (self.dayBetaChange==None):
            ...
            if len(bound[0]) != self.BetaChange*2+3:
                raise ValueError("Bound of Incorrect size")
                return False
        elif self.BetaChange!=0:
            if len(bound[0])==3:
        ...
        if self.BetaChange!=0:
            if self.isFitDayBetaChange == True:
                ...
                for i in range(self.BetaChange+1):
                    pos.append(self.beta[i])
                pos.append(self.gamma)
                pos.append(self.sigma)
                pos = np.array(pos)
                par=ps.backend.generators.create_swarm(20,dimensions=((self.BetaChange+1)*2)+1,bounds=self.bound)
                for i in range(5):
                    par.position[i]=pos
                
                optimizer = ps.single.GlobalBestPSO(n_particles=20, dimensions=((self.BetaChange+1)*2)+1, options=options,bounds=self.bound,init_pos=par.position)
                cost, pos = optimizer.optimize(self._objectiveFunction, 1000,n_processes=self.nCores)
                ...
                beta.append(pos[self.BetaChange*2])
                self.gamma = pos[self.BetaChange*2+1]
                self.sigma = pos[self.BetaChange*2+2]
                self.dayBetaChange = np.array(dayBetaChange)
                self.beta = np.array(beta)
                self.rmse = cost
                #self.cost_history = optimizer.cost_history
                ...
                optimizer = ps.single.GlobalBestPSO(n_particles=20, dimensions=self.BetaChange+3, options=options,bounds=self.bound)
                cost, pos = optimizer.optimize(self._objectiveFunction, 1000,n_processes=self.nCores)
                beta = []
                for i in range(len(self.BetaChange)+1):
                    beta.append(pos[i])
                self.gamma = pos[self.BetaChange+1]
                self.sigma = pos[self.BetaChange+2]
                self.dayBetaChange = np.array(dayBetaChange)
                self.beta = np.array(beta)
                self.rmse = cost
                self.cost_history = optimizer.cost_history
        else:
            optimizer = ps.single.GlobalBestPSO(n_particles=20, dimensions=3, options=options,bounds=self.bound)
            cost, pos = optimizer.optimize(self._objectiveFunction, 1000,n_processes=self.nCores)
            beta = [pos[0]]
            self.beta = np.array(beta)
            self.gamma = pos[1]
    ...
    def fit(self,x, y ,fittingByCumulativeCases=True, bound = ([0,1/7,1/6],[2,1/4,1/4]) ,stand_error=True, BetaChange=0,dayBetaChange = None ,mu = 1/(75.51*365),particles=100,itera=1000,c1=0.5,c2= 0.3, w= 0.9,k=5,norm=1):
        '''
        x = dias passados do dia inicial 1
        y = numero de casos
        bound = intervalo de limite para procura de cada parametro, onde None = sem limite
        ...
        if self.BetaChange!=0:
            if self.dayBetaChange==None:
                ...
                #cost, pos = optimizer.optimize(self._objectiveFunction, itera,n_processes=self.nCores)
                #__fitNBeta(self,dim,n_particles,itera,options,objetive_function,**kwargs)
                    pos,cost = self._Models__fitNBeta(dim=((self.BetaChange+1)*2)+1,n_particles=self.particles,itera=self.itera,options=options,objetive_function=  self._objectiveFunction,BetaChange= self.BetaChange,bound=self.bound)
                else:
                    optimizer = ps.single.LocalBestPSO(n_particles=particles, dimensions=((self.BetaChange+1)*2)+1, options=options,bounds=self.bound)
                    cost, pos = optimizer.optimize(self._objectiveFunction, itera,n_processes=self.nCores)
    ...
    def predict(self,numDays):
        ...
        if self.BetaChange==0:
            ...
            S,E,I,R = self.__cal_EDO_2(x,self.dayBetaChange,self.beta,self.gamma,self.mu,self.sigma)
        self.ypred = I+R
        self.NCpred = self._Models__changeCases(self.ypred)
        self.xpred = x
        self.S = S
    ...
    def getCoef(self):
        if self.BetaChange>0:
            ...
            var.append('sigma')
            valor.append(self.gamma)
            valor.append(self.mu)
            valor.append(self.sigma)
            return dict(zip(var, valor))
    ...
    def computeCI(self, times=500, level=0.95):
        ...
        if self.isCI:
            ...
            self.lI = self._Models__getConfidenceInterval(self.__bI, level,False)
            ...
            self.lGamma = self._Models__getConfidenceInterval(self.__bGamma, level,True)
            self.lSigma = self._Models__getConfidenceInterval(self.__bSigma, level,True)
        ...
        for i in range(0,len(casesSeries)):
            ...
            self.__bGamma.append(copia.gamma)            
            self.__bSigma.append(copia.sigma)
