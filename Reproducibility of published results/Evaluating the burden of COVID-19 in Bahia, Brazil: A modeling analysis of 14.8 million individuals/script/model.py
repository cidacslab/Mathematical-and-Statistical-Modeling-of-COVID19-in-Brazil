...
class SEIRHUD:
    ...
    def fit(self, x,y,d,pesoMorte = 0.5, kappa = 1/4,p=0.2,gammaA=1/3.5,gammaS=1/4,muH = 0.15,muU=0.4,xi = 0.53,omegaU = 0.29,omegaH=0.14 , bound = [[0,1/8,1/12,0,0.05],[2,1/4,1/3,0.7,0.25]],stand_error=False, beta2=True,day_mudar = None,paramPSO = {'options':{'c1': 0.3, 'c2': 0.3, 'w': 0.9,'k':5,'p':2},'n_particles':150,'iter':250}):
        '''
        ...
        if beta2:
            cost, pos = optimizer.optimize(self.objectiveFunction,paramPSO['iter'], x = x,y=df,d=dd,stand_error=stand_error,n_processes=self.numeroProcessadores, verbose = True)
        else:
            cost, pos = optimizer.optimize(self.objectiveFunction, paramPSO['iter'], x = x,y=df,d=dd,stand_error=stand_error,n_processes=self.numeroProcessadores, verbose = True)
            self.beta = pos[0]
            self.gammaH = pos[1]
    ...
    def predict(self,x):
        ...
        self.U = U
        self.R = R         
        return {"pred": self.ypred, "death": self.dpred, "hosp": self.H, "UTI": self.U, "exposed":self.E, "susceptible": self.S}
    ...
    def getCoef(self):
        if self.beta_variavel:
            return {'beta1':self.beta1, 'beta2':self.beta2,'dia_mudanca':self.day_mudar,'gammaH':self.gammaH,'gammaU':self.gammaU, 
                    'delta':self.delta,'h':self.h,'ia0':self.ia0,'is0':self.is0,'e0':self.e0}
        return {'beta':self.beta, 'gammaH':self.gammaH,'gammaU':self.gammaU, 
                    'delta':self.delta,'h':self.h,'ia0':self.ia0,'is0':self.is0,'e0':self.e0}
