...
SEIIHURD Fitting routine for data up to Sept 13 for Bahia.
...
df = pd.read_csv("../data/data_up_to_september.csv")
newcases = True
...
param = {'delta': 0.62, #asymptomatic infection correction
    ...
    'beta': [1.06, 0.63, 0.5], #infectivity matrix
    'tcut': [30., 60.], #time instants of change of the infectivity matrix
...
parnames = ['beta1', 'beta2', 'beta3', 'e0', 'ia0', 'is0', 'gammaH',	'gammaU', 't1', 't2', 'delta', 'h']
df = pd.DataFrame(np.array(pars_pso))
df.columns = parnames
df.to_csv('../Parameters_up_to_september.csv', index=False)
...
for i in range(4):
    ...
    ddf.columns = cols
    ddf.to_csv(names[i]+'_up_to_september.csv', index=False)
...
if newcases:
    for i in range(2,4):
        ...
        ddf = pd.DataFrame(np.array([mean,low,upp]).T)
        ddf.columns = cols
        ddf.to_csv(names[i]+'_cum_up_to_september.csv', index=False)
